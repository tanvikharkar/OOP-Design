...
public class PathingMain extends PApplet
{
   ...
   private boolean depthFirstSearch(Point pos, GridValues[][] grid, List<Point> path){
      // checking all possible paths to see if this is a valid grid cell
         ...
         grid[pos.y][pos.x] != GridValues.SEARCHED)
      {
         ...
         if (grid[pos.y][pos.x] == GridValues.GOAL)
         {
            ...
            // add current position to searched values
            grid[pos.y][pos.x] = GridValues.SEARCHED;

            // calculated based on given grid layout
            ...
            else if (depthFirstSearch(leftN, grid, path)){
               path.add(0, leftN);
               return true;
            }
            else if (depthFirstSearch(upN, grid, path)){
               path.add(0, upN);
               return true;
            }
            else if (depthFirstSearch(downN, grid, path)){
               path.add(0, downN);
               return true;
            }
         }
